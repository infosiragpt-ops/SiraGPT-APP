...
// Blueprint field type → Prisma scalar. Kept in sync with the coercions the
// generated route handlers apply (decimal/integer → number, boolean, etc.) so
// the DB column type matches what POST writes.
const PRISMA_TYPES = {
  id: 'String',
  string: 'String',
  text: 'String',
...
function buildApiRoute(model) {
  ...
  return [
    ...
    '',
    '// Next 15 statically caches route-handler GETs by default; force dynamic so',
    '// the list refreshes right after a POST.',
...
/**
 * Prisma schema (one `model` per entity) matching the accessors the generated
 * route handlers use. codegen OWNS this file so a direct caller of
 * `codegenFromBrief` gets a project where `prisma generate` succeeds.
 * @param {Array<{entity:string, fields:Array<{name:string,type:string}>}>} entities
 */
