...
| 9B | `ignore` | https://github.com/kaelzhang/node-ignore | `7.0.5` | MIT | 2025-05-31 | 237M last week | `.gitignore` filtering for GitHub Codex/RAG | Keeps generated and project-ignored files out of repo context without cloning | Some edge-case differences from native Git; built-in secret filters still take precedence | `backend/src/services/github-codex-connector.js`, tests/docs | Manual glob parsing, `git check-ignore` after clone | Small, dependency-free, permissive and purpose-built for `.gitignore` semantics |
| 9C | No new package | Existing official `octokit` client | Existing | MIT | Existing | Existing | GitHub Actions Intelligence | Lets Codex inspect CI runs, jobs, failed steps and sanitized logs | Logs can be large/noisy; strict truncation and redaction required | `backend/src/services/github-codex-connector.js`, `backend/src/routes/github-codex.js`, `app/codex/page.tsx`, tests/docs | `gh` CLI shelling, custom REST fetch | Reuses hardened Octokit without expanding supply-chain surface |
