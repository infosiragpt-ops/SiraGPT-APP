...
test('PLAN_QUOTAS: matches the cycle-46 ladder', () => {
  ...
  assert.equal(PLAN_QUOTAS.PRO, 500_000);
  assert.equal(PLAN_QUOTAS.PRO_MAX, 1_000_000);
...
test('quotaForPlan: returns the matching tier', () => {
  ...
  assert.equal(quotaForPlan('PRO'), 500_000);
  assert.equal(quotaForPlan('PRO_MAX'), 1_000_000);
