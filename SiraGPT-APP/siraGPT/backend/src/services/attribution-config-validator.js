...
const RULES = Object.freeze([
  { id: 'saliency_threshold_order', severity: 'error', description: 'live threshold must be > fading threshold',
    check: (e) => (Number(e.SIRAGPT_SALIENCY_LIVE_THRESHOLD) || 0.50) > (Number(e.SIRAGPT_SALIENCY_FADING_THRESHOLD) || 0.15) },
  { id: 'saliency_thresholds_in_range', severity: 'error', description: 'live and fading thresholds must be in (0, 1]',
    check: (e) => {
      const l = Number(e.SIRAGPT_SALIENCY_LIVE_THRESHOLD) || 0.50;
      const f = Number(e.SIRAGPT_SALIENCY_FADING_THRESHOLD) || 0.15;
  ...
  { id: 'reflection_thresholds_in_range', severity: 'error', description: 'reflection thresholds must be in (0, 1]',
    check: (e) => {
      const a = Number(e.SIRAGPT_REFLECTION_ACCEPT_THRESHOLD) || 0.65;
      const s = Number(e.SIRAGPT_REFLECTION_SOFT_THRESHOLD) || 0.45;
