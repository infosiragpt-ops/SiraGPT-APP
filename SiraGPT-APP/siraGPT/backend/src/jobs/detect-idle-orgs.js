...
async function run(opts = {}) {
  ...
  for (const key of existingFlagKeys) {
    const orgId = key.slice(KEY_PREFIX.length);
  ...
  for (const org of orgs) {
    ...
    const recent = await prisma.orgMembership.findFirst({
      where: { orgId: org.id, user: { lastActiveAt: { not: null }, deletedAt: null } },
  ...
  logger.info?.(
    `[detect-idle-orgs] done scanned=${scanned} flagged=${flagged.length} `
