...
describe("Native QA GitHub Actions artifact manifests", () => {
  ...
  it("publishes macOS and Windows QA artifacts with checksum manifests", () => {
    ...
    assert.match(workflow, /Windows Store AppX missing from desktop beta/)
    assert.match(workflow, /Windows Store package metadata missing from desktop beta/)
    assert.match(workflow, /Windows Store AppX checksum does not match its metadata/)
    assert.match(workflow, /-name '\*\.appx'/)
    assert.match(workflow, /-name 'windows-store-package\.json'/)
    assert.match(workflow, /Store ready: \\\`\$\{store_submission_ready\}\\\`/)
