...
describe("Signed native release workflow integrity", () => {
  ...
  it("verifies signed artifacts before publishing them", () => {
    ...
    assert.match(workflow, /name: Verify Google Play upload certificate/)
    assert.match(workflow, /ANDROID_PLAY_UPLOAD_SHA1:\s*\$\{\{\s*vars\.ANDROID_PLAY_UPLOAD_SHA1\s*\}\}/)
    assert.match(workflow, /ANDROID_PLAY_UPLOAD_SHA1 must match the upload certificate registered in Google Play/)
    assert.match(workflow, /npm run native:android:signing-fingerprint -- --aab="\$aab_path"/)
