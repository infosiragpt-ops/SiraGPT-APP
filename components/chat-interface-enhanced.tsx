...
export default function ChatInterface() {
  ...
  const handleSend = async () => {
    ...
    if (msg.toLowerCase().includes('generate image') || msg.toLowerCase().includes('create image')) {
      try {
        const imageUrl = await aiService.generateImage('ChatGPT', msg)

        // Add user message
        await addMessage(msg, files)
        ...
        const updatedMessages = [...currentChat.messages, aiResponse]
        // This would need to be handled by the chat context
