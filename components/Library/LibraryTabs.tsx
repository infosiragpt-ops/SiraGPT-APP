...
const LibraryTabs: React.FC = () => {
    ...
    const renderGrid = (items: LibraryItem[], type: 'image' | 'video') => {
        ...
        return (
            ...
            <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
                {isEmpty ? (
                    <div className="col-span-full flex flex-col items-start text-left text-gray-500 dark:text-gray-400 py-4">
                    ...
                    items.length > 0 ? (
                        items.map((item) => (
                            <div
                                key={item.id}
                                className="relative group rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden hover:shadow-md transition"
                            >
                                {/* Preview */}
                                {type === 'image' ? (
                                    <Image
                                ...
                                <button
                                    onClick={() => handleDownload(item.url, `file-${item.id}.jpg`)}
                                    className="absolute bottom-2 right-2 bg-black/60 hover:bg-black/80 text-white p-2 rounded-full opacity-0 group-hover:opacity-100 transition"
                                >
