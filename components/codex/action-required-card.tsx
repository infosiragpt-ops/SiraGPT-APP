...
export function ActionRequiredCard({ title, rawError, blockedCapabilities, remediationUrl }: ActionRequiredCardProps) {
  ...
  return (
    <div className="my-2 rounded-xl border border-red-500/30 bg-red-500/10 p-3">
      <div className="text-sm font-semibold text-red-300">🔴 Acción requerida de su parte</div>
      ...
      {blockedCapabilities.length > 0 && (
        <div className="mt-2 text-xs text-zinc-400">
          <span className="font-medium text-zinc-300">Capacidades bloqueadas:</span>
      ...
      {remediationUrl && (
        ...
        >
          Remediar <ExternalLink className="h-3.5 w-3.5" />
        </a>
      )}
    </div>
  )
