  ...
  if (item.kind === "plan") {
    return (
      <div className="my-2 rounded-lg border border-violet-500/20 bg-violet-500/5 p-3 text-sm">
        <div className="font-semibold text-violet-200">Plan propuesto</div>
        <div className="mt-1 text-zinc-300">{item.architecture}</div>
        <div className="mt-1 text-xs text-zinc-400">{item.pages.length} páginas · {item.components.length} componentes · {item.tasks.length} tareas</div>
      </div>
    )
  }
  if (item.kind === "checkpoint") {
    return <div className="my-2 rounded-lg border border-white/10 bg-white/5 p-3 text-sm"><span className="font-medium">Checkpoint</span> · <code className="text-xs">{item.commitSha?.slice(0, 7)}</code> — {item.title}</div>
  }
  if (item.kind === "summary") {
    const m = item.metrics || {}
    return <div className="my-2 rounded-lg border border-white/10 bg-white/5 p-3 text-xs text-zinc-300">Trabajó · {m.actionsCount ?? 0} acciones · +{m.additions ?? 0} −{m.deletions ?? 0}</div>
  }
  if (item.kind === "action_required") {
    return (
      <div className="my-2 rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-sm">
        <div className="font-semibold text-red-300">🔴 Acción requerida de su parte</div>
...
function renderItem(item: TimelineItem, cardRenderer?: CodexRunTimelineProps["cardRenderer"]): React.ReactNode {
  switch (item.kind) {
    case "narrative":
      return (
    ...
    default: {
      ...
      return <React.Fragment key={item.id}>{custom != null ? custom : <FallbackCard item={item} />}</React.Fragment>
    }
  }
}
...
export function CodexRunTimeline({ state, cardRenderer, className }: CodexRunTimelineProps) {
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col">
      ...
      >
        {state.items.map((item) => renderItem(item, cardRenderer))}
      </div>
      ...
      {stick.showPill && (
        ...
        >
          <ArrowDown className="h-3.5 w-3.5" /> Ir a lo último
        </button>
      )}
    </div>
  )
