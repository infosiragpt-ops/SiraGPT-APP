...
<body>
  ...
  <div class="slide">
    <h2>${slide.title}</h2>
    <ul>
      ${(Array.isArray(slide.content) ? slide.content : [slide.content]).map(point => `<li>${point}</li>`).join('')}
    </ul>
  </div>
  `).join('')}
</body>
    ...
    return (
        <div className="fixed inset-0 bg-slate-900 z-50 flex flex-col animate-in fade-in">
            {/* Header */}
            <div className="bg-slate-800/80 backdrop-blur-lg border-b border-slate-700 px-4 py-3 flex items-center justify-between">
                <h1 className="text-lg font-bold text-white truncate">{presentation.title}</h1>
            ...
            <div className="flex-1 flex items-center justify-center p-4">
                <div className="w-full max-w-5xl aspect-video bg-white rounded-lg shadow-2xl overflow-hidden flex flex-col">
                    {currentSlide === 0 && titleSlide ? (
                        ...
                        <div className="h-full flex flex-col justify-center p-12 text-slate-800">
                            <h2 className="text-4xl font-bold mb-6 text-blue-900 pb-2 border-b-2 border-blue-100">{slide.title}</h2>
                            <ul className="space-y-4">
                                {(Array.isArray(slide.content) ? slide.content : [slide.content]).map((point, idx) => (
