 ...
 * composer, tabs, errors) to every messages/<locale>.json. Spanish is the
 * SOURCE; English is the base fallback; the major locales get hand
 * translations and every other locale gets the English strings (next-intl's
 * deep merge already falls back to en, so shipping en text instead of holes
 * keeps the drift checker green).
 *
 * Idempotent: re-running overwrites ONLY the `codex` namespace, preserving any
 * other keys (and any manual edits to other namespaces) in each file.
 */
