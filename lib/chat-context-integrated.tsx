...
export function ChatProvider({ children }: { children: React.ReactNode }) {
  ...
  // Persist model selection across sessions (internal — no UI change)
  useEffect(() => {
    if (selectedModel && selectProvider) {
      savePersistedModelSelection(selectedModel, selectProvider)
    }
  }, [selectedModel, selectProvider])
  ...
  const initializeChat = async () => {
    ...
    try {
      ...
      const persisted = loadPersistedModelSelection()
      const picked = pickModelFromCatalog(modelsResponse.models, persisted)
      if (picked) {
        devLog("model selected:", picked, persisted ? "(restored)" : "(default)")
        setSelectedModel(picked.model)
        setSelectedProivder(picked.provider)
      }
