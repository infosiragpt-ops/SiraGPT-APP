...
export const CODE_COMPANY_ASSOCIATION_CHANGED_EVENT = "siragpt:code-company-association-changed"

export function notifyCompanyAssociationChanged(): void {
  if (typeof window === "undefined") return
  window.dispatchEvent(new CustomEvent(CODE_COMPANY_ASSOCIATION_CHANGED_EVENT))
}
...
export function CodeWorkspaceProvider({ children }: { children: React.ReactNode }) {
  ...
  const switchCodexWorkspace = React.useCallback(
    async (target: { id: string; name: string; kind: "local-folder" | "project"; projectId?: string }) => {
      if (target.id.startsWith("codex:")) {
        try {
          const project = await codexApi.getProject(target.id.slice(6))
          ...
          toast.error(error instanceof Error ? error.message : "Proyecto no disponible.")
        }
        return
      }
