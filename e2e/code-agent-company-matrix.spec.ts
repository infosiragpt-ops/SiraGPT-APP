...
test("modern office city renders a moving day and night environment", async ({ page }, testInfo) => {
  ...
  await expect.poll(async () => Number(await canvas.getAttribute("data-city-glass-facade-count"))).toBeGreaterThanOrEqual(70)
  await expect.poll(async () => Number(await canvas.getAttribute("data-city-terrace-amenity-count"))).toBeGreaterThanOrEqual(18)
  await expect.poll(async () => Number(await canvas.getAttribute("data-city-tallest-building-height"))).toBeGreaterThanOrEqual(46)
