...
/**
 * /auth error boundary. reset() remounts the segment.
...
export default function RouteError({
  ...
  useEffect(() => {
    try {
      console.warn("[auth] route error", error?.digest || error?.name)
    } catch {
      /* ignore */
    }
  }, [error])
  ...
  return (
    <div className="flex min-h-[50vh] items-center justify-center p-4">
      <div className="mx-auto max-w-md rounded-xl border border-border/60 bg-card p-6 text-center shadow-lg">
        ...
        <h1 className="mb-2 text-xl font-semibold">No se pudo cargar el acceso</h1>
        <p className="mb-4 text-sm text-muted-foreground">
          Ocurrió un error al mostrar esta pantalla. Reintentar remonta la ruta. No se muestra ningún dato de la cuenta.
        </p>
        {error.digest ? (
