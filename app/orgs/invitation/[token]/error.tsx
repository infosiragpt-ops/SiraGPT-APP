...
/**
 * /orgs/invitation error boundary. reset() remounts the segment.
...
export default function RouteError({
  ...
  useEffect(() => {
    try {
      safeRouteErrorLog("orgs-invite", error)
    } catch {
      /* ignore */
    }
  }, [error])
  ...
  return (
    <div className="flex min-h-[50vh] items-center justify-center p-4">
      <div className="mx-auto max-w-md rounded-xl border border-border/60 bg-card p-6 text-center shadow-lg">
        ...
        <h1 className="mb-2 text-xl font-semibold">No se pudo cargar la invitación</h1>
        <p className="mb-4 text-sm text-muted-foreground">
          Ocurrió un error al mostrar la invitación. Reintentar remonta la ruta. El token no se muestra.
        </p>
        {error.digest ? (
