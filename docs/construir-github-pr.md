...
3. Write something like:
   ...
   **«abre un PR en owner/repo que añada un README con instrucciones de arranque»**
   ...
   For **«dame la web en local»** (clone + preview on the server, not the
   user's phone), the agent uses `project_clone_repo` + `project_preview_start`
   (`docs/agentes-local-preview.md`). That path is not this OAuth PR flow.
...
5. If GitHub is not connected, the tool answers in Spanish and points to
   `/conexiones` (`E_GITHUB_CONNECT`). Sira never invents a token. The isolated
   workspace is not the user's machine. Local preview of a public repo does
   not require OAuth — see `docs/agentes-local-preview.md`.
