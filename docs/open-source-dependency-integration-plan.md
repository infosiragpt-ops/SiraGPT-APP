...
| P1 integrada | `ignore` | https://github.com/kaelzhang/node-ignore | `7.0.5` | MIT | Aplicar reglas `.gitignore` en Codex/RAG | Evita descargar/indexar archivos generados o ignorados por el repo sin clonar codigo | Diferencias menores frente a Git en casos extremos; se mantiene filtro built-in para secretos | `backend/src/services/github-codex-connector.js`, tests/docs | Glob manual, `git check-ignore` | Pequeña, sin dependencias, MIT y diseñada exactamente para semantica `.gitignore` |
