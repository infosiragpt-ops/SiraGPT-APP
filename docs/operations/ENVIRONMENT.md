...
| `AGENTES_CODING_V2` | Coding Agents flag (`1`/`true`/`on`). Query `GET /api/agentes-coding/health`. Default OFF in production; does not change `/agentes` UX when off. Phase 2a sessions: `docs/agentes-coding-sandbox.md`. Phase 3a IDE shell (only when `health.enabled`): `docs/agentes-coding-ide.md`. Do not set on Lenovo from this PR | (off) |
