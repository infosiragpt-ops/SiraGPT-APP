...
- Rubrica motor: `/opt/siragpt/docs/mejoras/benchmark-capacidades.md` (v3H43 live, 2026-08-21 09:57 Lima).
- Ultima ola live: `/opt/siragpt/docs/mejoras/ola-3h-20260821-0957.md` (3H43, tool arg 32KiB cap + JSON unescaped-newline repair + optional "null" coerce + max 16 SSE buffers + compact pins+last 3 users + refuse write if dest dir missing + ceil tokens on cancel + ECONNRESET cancelled + queue wait 60s 503 + embedding dim skip + mid-stream stall 20s + UTF-16 NUL strip + tool name max 64 + recursive same-name >8 + skip completed plan steps + gzip ckpt >64KiB + Last-Event-ID int-only + glob file 1MiB + redact Authorization Bearer + refuse host_bash computer-only + subagent inherit remaining steps + concat split tool_call fragments + never retry 402 + combined stdio 96KiB + ping if last write >15s + unicode slash homoglyph + lock owner pid + Prisma disconnect retryable + default tool timeout 30s + close SSE then settle credits, frontend untouched)
