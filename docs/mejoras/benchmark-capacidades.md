...
| SSE reconnect resume | replay events; persist Last-Event-ID; reject last > head **and going backwards** | **fail-closed** — 3H64/3H65 + **3H67** drop comments / stale 2 min / cap 64 / restore cursor / parse int-only / `event: done` | 0 re-exec; 0 replay desde seq inválido o hacia atrás | `3H67-D-001` / `3H65-H-001` / `3H65-G-001` + 3H64-D-001 |
