...
| Memory recovery (pgvector pins) | facts `pin`/`score≥0.85` restaurados | **fail-closed** — `recoverPgvectorPinsClosed` en compact + `recall`/`searchableMemoryHook` | 100 % pins recuperados post-compact | `3H62-H-001` / `3H62-M-001`; live: `retrieve_memory` + compact |
...
| SSE reconnect resume | replay events; persist Last-Event-ID; reject last > head | **fail-closed** — `persistLastEventIdClosed` + `resumeGenerateFromPersistedIdClosed` (inclusivo si hay ring; drop listeners) | 0 re-exec; 0 replay desde seq inválido; cursor sobrevive restart | `3H62-E-001` / `3H62-F-001` / `3H62-Q-001` |
...
| Latencia p50/p95 first-token y end-of-turn | histogram scripted (nunca Flash inventado) | **fail-closed** — `observeTurnLatencyClosed` en loop (first-token + turn-end) y generate finally; tests con fake timers | p50/p95 reales bajo carga VPS | `3H62-J-001`; live: `adapterLatencySnapshot()` |
