...
## Fase 2 (PRs #744, #745): Jev dentro del harness
...
* **Guardarraíl de herramientas** (`rlcd/jev-tool-guard.js`, PR #745): en el gate de permisos del harness (`agent-harness/event-stream.js`), toda llamada que no sea de solo lectura ni generación de medios se evalúa (Choice riesgo + Noul irreversible + Noul «¿lo pidió el usuario?»). Con riesgo calibrado ≥ `SIRAGPT_RLCD_JEV_TOOL_CONFIRM` (0,7) o acción no pedida ≥ `_TOOL_UNREQUESTED` (0,8) la llamada pasa por la tarjeta de permiso con el motivo de Jev. Kind `tool_risk`; la respuesta del usuario puntúa la decisión (`guard_needed` / `guard_unneeded`). Evento SSE `tool_risk_assessed`.
* **Selector de skills** (`rlcd/jev-skill-picker.js`, PR #745): Choice sobre el catálogo visible de skills (+ `none`); las ganadoras van primero como RECOMENDADA en `run_skill`. Kind `skill_route`, puntuado por el resultado de `run_skill`.
* Flags: `SIRAGPT_RLCD_JEV_JUDGE`, `_JEV_LANE_STEERING`, `_JEV_LANE_VETO`, `_JEV_TRIAGE`, `_JEV_COMPUTE`, `_JEV_MODEL_STEERING` (advisory), `_JEV_SATISFACTION`, `_JEV_TOOL_GUARD`, `_JEV_SKILL_PICKER`; umbrales en `rlcd/config.js` (visibles en `/admin/rlcd`).
