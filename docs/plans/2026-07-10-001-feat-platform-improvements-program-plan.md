...
- **Approach:** U0 is a reviewed one-off release operation and must complete before schema-bearing units. Inventory the live schema against `schema.prisma`, identify the migration history already represented in production, and apply Prisma's supported baseline resolution under explicit human review without replaying historical DDL. Application boot and `--migrate-only` never perform this history mutation. Remove the temporary no-schema U1 equivalence compatibility only after a disposable production-shaped database proves `migrate deploy` succeeds from the reviewed baseline.
