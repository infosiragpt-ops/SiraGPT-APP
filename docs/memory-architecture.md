...
| Búsqueda agéntica | tools `memory_search`, `chat_history_search`, `connector_search` | `memory_search`: grep léxico (acentos/mayúsculas plegados) sobre toda la memoria; el peldaño vectorial (`long-term-memory.recallFacts`) solo se suma cuando el corpus supera `SIRAGPT_MEMORY_GREP_MAX_CHARS` (24k). `chat_history_search`: FTS Postgres (`content_tsv`) sobre los chats anteriores del usuario (`memory/chat-history-search.js`), con fallback ILIKE. `connector_search` (`memory/connector-search.js`): Google Drive y Gmail con los tokens ya conectados del usuario (metadatos + fragmento, nunca credenciales), cada fuente falla suave. |
