...
| `OpenHands/software-agent-sdk` | MIT | Workspace task loop, ACP-style worker | **pattern only**. Do not take the PolyForm enterprise tree. Phase 4a: native plan/tool/result loop | `backend/src/services/agentes-coding/harness/` | already fused (pattern) |
...
| `cline/cline` | Apache-2.0 | Human-in-the-loop permission UX (ask / once / always / reject) | **HITL patterns only**. Not the VS Code extension, not the desktop UI. Phase 4b: native pause/resolve on the harness run | `backend/src/services/agentes-coding/harness/permissions.js` | already fused (pattern) |
| in-repo `bullmq` + `doc-sandbox/queue` + `codex/run-queue` | MIT (BullMQ) | Durable HITL / long tool loops | **pattern only** (enqueue, attempts:1, flag-gated worker). No document-sandbox tables, no Temporal dump. Phase 4c | `backend/src/services/agentes-coding/harness/jobs/` | already fused (pattern) |
| in-repo native client + brand labels | existing | Default coding-harness completion | **reuse** (flag on, no inject). Brand aliases only in API JSON. Phase 4d | `backend/src/services/agentes-coding/harness/llm.js` | already fused (reuse) |
| OpenSandbox / E2B lifecycle (already fused) | Apache-2.0 (pattern) | Persist session workspace across host/process restart | **pattern only**. Bind-mount under a dedicated data dir; no Daytona, no named-volume socket tricks. Phase 4e | `backend/src/services/agentes-coding/coding-sandbox/volume.js` | already fused (pattern) |
| in-repo todo-app golden | existing | API-only CI smoke (memory + injectable LLM) | **native**. Tiny `package.json` + `src/app.js` fixture. No SWE-bench dump, no real model. Phase 4f | `backend/tests/fixtures/agentes-coding-todo/` | already fused (pattern) |
...
| `SWE-bench/SWE-bench` | MIT | Coding-agent benchmark corpus | **pattern / fixture idea only**. Phase 4f native golden (not the Python corpus) | `backend/tests/fixtures/agentes-coding-todo/` | already fused (pattern) |
