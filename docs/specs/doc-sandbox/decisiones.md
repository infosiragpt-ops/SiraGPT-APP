...
## D30. Limpieza progresiva de evidencias privadas sin certificado parcial

- Contexto: el camino de borrado reúne todas las claves antes de purgar. El listado acotado a 10 000 puede rechazar un prefijo legítimo agregado de varios documentos/intentos y no borrar siquiera las claves conocidas. Este riesgo debe resolverse antes de ampliar la retención de evidencias fallidas de D27.
- Decisión: conservar `list()` y su límite para consumidores existentes; añadir un iterador de páginas para cleanup, con prefijo validado, máximo 1000 claves por página, tokens opacos y control de ciclos, plazo compartido y reintentos exclusivamente del LIST actual. Borrar conocidas pendientes y después cada página; antes de DELETE, registrar sus claves bajo bloqueo del tombstone con gracia vencida y ámbito exacto. Sólo confirmar DELETE efectivamente respondidos, en lotes de 100 y al abandonar el tramo. No filtrar objetos de LIST por el journal de purgados: una reaparición reabre su obligación de limpieza. Exigir recorrido completo, señal vigente y una comprobación adicional desde el prefijo vacío antes de finalizar.
- Consecuencias: un fallo o plazo vencido conserva progreso y obligación de limpieza para la siguiente pasada, sin reiniciar el cursor del recorrido actual ni certificar borrado parcial. Los 30 segundos limitan IO de storage, no garantizan un SLA duro para transacciones PostgreSQL. Prueba real PostgreSQL/MinIO de más de 10 000 objetos, separada de la cobertura unitaria; no cambia catch, validador, retención, aislamiento ni esquema. API contrastada con [ListObjectsV2 oficial](https://docs.aws.amazon.com/AmazonS3/latest/API/API_ListObjectsV2.html). Dos fallos pre-fix reproducidos y 79/79 integraciones finales aprobadas; 10 003 objetos purgados en tres pasadas, respuesta DELETE perdida y escritura posterior a LIST no truncado verificadas. No es aceptación F1 ni despliegue; evidencia y límites en [cleanup-pagination-release-20260906.md](cleanup-pagination-release-20260906.md).

## D31. Evidencia fallida privada y transacción única, tras reproducir D27

- Contexto: el ensayo real sobre el catch extraído reprodujo el informe guardado sin diff (3/4); la limpieza de D30 permite avanzar sin el límite previo de 10000 claves acumuladas. D27 permanece como registro del diseño preliminar, no se reescribe como prueba ejecutada.
