...
| **F7** | SiraComputer (multimodal + desktop VM) | Visión en el loop, voz, y computer-use sobre un `DesktopProvider` híbrido (E2B más tarde, LocalGvisor más tarde). **F7.0 es la única sub-fase activa ahora.** Spec: `F7_SIRACOMPUTER_MASTER_SPEC.md`. El orquestador live (`services/computer-orchestrator`, #484) se CONSERVA — F7 AÑADE una capa, no lo sustituye. ORM = Prisma (Drizzle no entra). | 4 pw (F7.0–F7.8) | F7.0: `docker build` de `sira-desktop` + DCP `/health`+`/screenshot` (skip honesto sin Docker). Sub-fases posteriores: ver spec §21 |
...
|---|---|---|
| **F7.0** | `DesktopProvider` (create/destroy/health/screenshot) + imagen `sira-desktop` (Xvfb+openbox+x11vnc/noVNC+xdotool+scrot+DCP `:9000`) | **activa** |
| F7.1 | `E2BDesktopProvider` real + warm pool | no |
| F7.2 | Session manager / acquire SLO | no |
