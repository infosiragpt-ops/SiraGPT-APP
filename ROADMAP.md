...
| **F7** | SiraComputer (multimodal + desktop VM) | Visión en el loop, voz, y computer-use sobre un `DesktopProvider` híbrido. **F7.0 COMPLETED**. **F7.1 COMPLETED**. **F7.2 COMPLETED**. Spec: `F7_SIRACOMPUTER_MASTER_SPEC.md`. El orquestador live (`services/computer-orchestrator`, #484) se CONSERVA. ORM = Prisma. | 4 pw (F7.0–F7.8) | F7.0 ✅ CI `desktop-f70`. F7.1 ✅ `desktop-f71`. F7.2 ✅ CI `desktop-f72` (run 33214651334). F7.3+: ver spec §21 |
...
| **F7.1** | `E2BDesktopProvider` real + warm pool (`DesktopSessionManager` in-memory, acquire p50 < 800 ms) | **COMPLETED** |
| **F7.2** | Full DCP + authenticated WS proxy + DesktopScreen first frame (spec §21 / §22.2) | **COMPLETED** (CI 33214651334) |
| F7.3 | CU-loop (visión → acción) | no |
| F7.4 | Handoff FSM (login/captcha; beyond DCP input_mode) | no |
| F7.5 | — reserved | no |
