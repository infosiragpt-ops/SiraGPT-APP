...
| **F7** | SiraComputer (multimodal + desktop VM) | Visión en el loop, voz, y computer-use sobre un `DesktopProvider` híbrido. **F7.0 COMPLETED**. **F7.1 COMPLETED**. **F7.2** DCP + WS + DesktopScreen (this PR). Spec: `F7_SIRACOMPUTER_MASTER_SPEC.md`. El orquestador live (`services/computer-orchestrator`, #484) se CONSERVA. ORM = Prisma. | 4 pw (F7.0–F7.8) | F7.0 ✅ CI `desktop-f70`. F7.1 ✅ `desktop-f71`. F7.2 `desktop-f72` (DCP + WS auth, sin Docker/E2B). F7.3+: ver spec §21 |
...
| **F7.2** | Full DCP + authenticated WS proxy + DesktopScreen first frame (spec §21 / §22.2) | **this PR** |
