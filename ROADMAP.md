...
| **F7** | SiraComputer (multimodal + desktop VM) | Visión en el loop, voz, y computer-use sobre un `DesktopProvider` híbrido (E2B más tarde, LocalGvisor más tarde). **F7.0 COMPLETED** (CI provision gate verde). Siguiente: F7.1 (no iniciada). Spec: `F7_SIRACOMPUTER_MASTER_SPEC.md`. El orquestador live (`services/computer-orchestrator`, #484) se CONSERVA. ORM = Prisma. | 4 pw (F7.0–F7.8) | F7.0 ✅ CI `desktop-f70`: `docker build` + DCP `/health`+`/screenshot`. F7.1+: ver spec §21 |
