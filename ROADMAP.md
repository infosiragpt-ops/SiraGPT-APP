...
| **F7** | SiraComputer (multimodal + desktop VM) | Visión en el loop, voz, y computer-use sobre un `DesktopProvider` híbrido. **F7.0–F7.3 COMPLETED**. **F7.4 IN_PROGRESS** (handoff FSM / leak gate). Spec: `F7_SIRACOMPUTER_MASTER_SPEC.md`. El orquestador live se CONSERVA. ORM = Prisma. | F7.0–F7.8 | F7.0–F7.3 ✅. F7.4 `desktop-f74` (sin Docker/E2B). F7.5+ no iniciada |
...
| **F7.2** | Full DCP + authenticated WS proxy + DesktopScreen first frame (spec §21 / §22.2) | **COMPLETED** |
| **F7.3** | CU-loop (visión → acción) | **COMPLETED** |
| **F7.4** | Handoff FSM (login/captcha; HARD leak gate) | **IN_PROGRESS** |
| F7.5 | Egress allowlist — no iniciar | no |
