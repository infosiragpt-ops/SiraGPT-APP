...
| **F7** | SiraComputer (multimodal + desktop VM) | Visión en el loop, voz, y computer-use sobre un `DesktopProvider` híbrido. **F7.0 COMPLETED**. **F7.1 IN_PROGRESS** (E2B real + warm pool; gate de provision pendiente de verificación). Spec: `F7_SIRACOMPUTER_MASTER_SPEC.md`. El orquestador live (`services/computer-orchestrator`, #484) se CONSERVA. ORM = Prisma. | 4 pw (F7.0–F7.8) | F7.0 ✅ CI `desktop-f70`. F7.1: `desktop-f71` acquire p50 < 800 ms + reaper (sin Docker/E2B). F7.2+: ver spec §21 |
...
### F7 sub-fases (SiraComputer)

Fuente de verdad: `F7_SIRACOMPUTER_MASTER_SPEC.md`. Implementar SOLO la sub-fase
activa en `STATE.md`. F7.1–F7.8 no se adelantan.

| Sub | Objetivo | En esta PR |
|---|---|---|
| **F7.0** | `DesktopProvider` (create/destroy/health/screenshot) + imagen `sira-desktop` (Xvfb+openbox+x11vnc/noVNC+xdotool+scrot+DCP `:9000`) | **COMPLETED** (CI) |
| **F7.1** | `E2BDesktopProvider` real + warm pool (`DesktopSessionManager` in-memory, acquire p50 < 800 ms) | **IN_PROGRESS** |
| F7.2 | Session manager acquire SLO / WS (spec §21) — **no iniciar** | no |
| F7.3 | WS proxy (noVNC hacia el usuario) | no |
| F7.4 | CU-loop (visión → acción) | no |
| F7.5 | Handoff FSM (login/captcha) | no |
| F7.6 | LocalGvisor flags completos (`runsc`, `--network none`) | no |
| F7.7 | Tablas Prisma `DesktopSession` | no |
| F7.8 | Panel frontend | no |
