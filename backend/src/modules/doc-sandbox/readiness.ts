import { DocumentProbeLifetime } from './probe-lifetime';
export { waitForDocumentOperation } from './probe-lifetime';
...
export function createDocumentWorkerReadinessProbe(queue: ReadinessQueue, worker: ReadinessWorker,
  ...
  const lifetime = new DocumentProbeLifetime(invalidate);
  return {
    async check(signal: AbortSignal): Promise<boolean> {
      if (lifetime.closed || signal.aborted || !worker.isRunning()) return false;
      return lifetime.run(async () => {
        ...
        clients.forEach(client => lifetime.observe(client));
        ...
        return !lifetime.closed && worker.isRunning() && replies.every((reply) => reply.status === 'fulfilled' && reply.value === 'PONG') &&
          clients.every((client) => client.status === 'ready');
      }, signal, timeoutMs);
    },
    close(): void { lifetime.close(); },
  };
}
