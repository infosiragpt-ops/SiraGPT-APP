...
export function createDocumentModelPolicy(
  ...
  return async (requestedModel, userPlan) => {
    ...
    const configured = matching.length === 1 ? matching[0]! : null;
    try {
      ...
      // Unlisted picker models use the mechanical engine path. Identity is
      // recorded on the job; it must not refuse verified admission.
      return configured ?? 'mechanical';
    } catch (cause) {
