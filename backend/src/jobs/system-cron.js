...
// Daily FREE plan call reset — midnight UTC
const FREE_DAILY_QUOTA_SCHEDULE = process.env.SYSTEM_CRON_FREE_DAILY_QUOTA_SCHEDULE || '0 0 * * *';
...
function start(opts = {}) {
  ...
  // FREE plan daily call quota reset (00:00 UTC)
  let freeDailyQuotaRunning = false;
  const freeDailyQuotaMeta = {};
  const freeDailyQuotaTask = cron.schedule(
    FREE_DAILY_QUOTA_SCHEDULE,
    async () => {
      if (freeDailyQuotaRunning) {
        logger.warn?.('[system-cron] skip reset-free-daily-quota — previous run still active');
        return;
      }
      freeDailyQuotaRunning = true;
      const finish = recordRun(freeDailyQuotaMeta, 'reset-free-daily-quota');
      let runErr = null;
      try {
        // eslint-disable-next-line global-require
        const job = require('./reset-free-daily-quota');
        const runWithRetry = wrapWithRetry(() => job.run({ logger }), {
          onRetry: ({ attempt, delayMs, reason }) =>
            logger.warn?.(`[system-cron] reset-free-daily-quota retry ${attempt} in ${delayMs}ms (${reason})`),
        });
        const res = await runWithRetry();
        logger.info?.(`[system-cron] reset-free-daily-quota done: ${JSON.stringify(res)}`);
      } catch (err) {
        runErr = err;
        logger.error?.(`[system-cron] reset-free-daily-quota failed: ${err && err.message}`);
      } finally {
        freeDailyQuotaRunning = false;
        finish(runErr);
      }
    },
    { scheduled: false, timezone: 'UTC' },
  ...
  tasks.push({
    name: 'reset-free-daily-quota',
    schedule: FREE_DAILY_QUOTA_SCHEDULE,
    task: freeDailyQuotaTask,
    meta: freeDailyQuotaMeta,
  });
