...
/**
 * Bounded computer control helpers.
...
function createRepeatGuard({ limit = REPEAT_ACTION_LIMIT } = {}) {
  ...
  return {
    ...
    record(name, args) {
      ...
      const info = { fingerprint, repeated, count: fingerprints.length, name, args };
      logStep(info);
      return info;
    },
  };
        ...
        if (String(tool.name || '').startsWith('computer_')) {
          ...
          if (repeated) {
            return JSON.stringify({
              ...
              message: 'same computer action repeated 3 times; aborting control loop',
            });
          }
        }
        try {
          ...
          return JSON.stringify({
            ok: false,
            error: 'computer_action_failed',
            message: String(err && (err.detail || err.message) || err || 'computer_action_failed'),
          });
        }
      },
    };
