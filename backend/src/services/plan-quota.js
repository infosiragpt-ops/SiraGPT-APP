 ...
 *
 *   FREE plan:
 *     `monthlyCallLimit` is a REMAINING-call counter (legacy column
 *     name). Resets daily at UTC midnight via free-daily-quota.js.
 *     New FREE users start at 3 calls/day.
 *
...
const { ensureFreeDailyQuota } = require('./free-daily-quota');

const FREE_CALL_LIMIT = FREE_DAILY_CALL_LIMIT;
...
async function tryConsumePlanQuota({ userId, prisma, user } = {}) {
  ...
  if (user.plan === 'FREE') {
    await ensureFreeDailyQuota(userId, prisma);
