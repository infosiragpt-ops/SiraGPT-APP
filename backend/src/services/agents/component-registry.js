...
const COMPONENTS = [
  {
    ...
    backing_modules: [
      ...
      "backend/src/services/cira/task-envelope-schema.js",
      "backend/src/services/cira/intent-taxonomy.js",
      "backend/src/services/cira/task-envelope-builder.js",
      "backend/src/services/cira/frames.js",
      "backend/src/services/cira/engine.js",
      "backend/src/services/cira/tool-registry.js",
      "backend/src/services/cira/intent-prompts.js",
      "backend/src/services/cira/validator-engine.js",
      "backend/src/services/cira/runtime.js",
    ],
    acceptance_criteria: [
      ...
      "Cira engine 6-step pipeline: Intent Engine → Planner → Tool Runtime → Artifact Engine → Validator → Response Builder. Composes the envelope, all 5 frames and the response in one runUserMessage() call; injects toolDispatcher and artifactRenderer for wet-runs — IMPLEMENTED",
      "Cira Tool Registry (MASTER_SPEC §11-12): typed CiraTool contracts (name, inputSchema, category, riskLevel, permissionsRequired, timeoutMs, retryable, requiresHumanConfirmation, execute) with 60+ default tools across document/spreadsheet/presentation/pdf/svg/landing/research/code/image/video/validator categories. Permission enforcement, timeout guard, requires_confirmation, integrity check — IMPLEMENTED (default executors are deterministic stubs; production replaces them via reg.register())",
      "Cira Validator Engine (MASTER_SPEC §5/cira-core/validators/): 5 validator families (artifact / source / code / document / safety) with 40+ deterministic checks. composeValidationFrame() aggregates and decides ready_to_deliver — IMPLEMENTED",
      "Cira intent + planner + validator system prompts (MASTER_SPEC §8-9): structured-outputs requests with json_schema response_format, vendor-agnostic — IMPLEMENTED",
      "Cira Tool Runtime: drives a workflow_graph through the registry, collects tool results, builds ArtifactFrame, runs the validator engine, produces ValidationFrame with ready_to_deliver verdict. Dry-run mode + wet-run mode + permission scopes + per-tool args — IMPLEMENTED",
