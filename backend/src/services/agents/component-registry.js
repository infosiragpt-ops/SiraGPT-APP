...
const COMPONENTS = [
  {
    ...
    backing_modules: [
      ...
      "backend/src/services/sira/hybrid-retrieval.js",
      "backend/src/services/sira/document-pipeline-registry.js",
      "backend/src/services/sira/llm-observability.js",
      "backend/src/services/sira/eval-harness.js",
    ],
    acceptance_criteria: [
      ...
      "Integration Stack v2 (26 backend capability layers, 200+ mapped libraries): typed adapter contracts + deterministic stubs for model gateway, agent SDK, durable orchestration, hybrid RAG, document intelligence, DOCX/XLSX/PPTX/PDF artifact engines, render/preview, scientific typesetting, structured outputs, MCP, compliant browser automation, database intelligence, ETL, BI studio, full-stack web builder, design compiler, sandbox, QA/evals, observability, events, cloud-native, security governance and chat runtime. resolveExecutionStack() maps Cira envelopes / intents / tools / formats into validation gates, security gates and required libraries. dependencyReadiness() verifies installed packages, configured environment bindings and wet-run blockers so Cira never claims unavailable tooling — IMPLEMENTED",
      ...
      "LiteLLM-style internal gateway: OpenAI-shaped route contract, provider normalization, retry/fallback policy, cost budget guard, provider trace, and explicit fallback authorization; no silent model switch by default — IMPLEMENTED",
      ...
      "Sira Hybrid Retrieval (recommended-stack §RAG): production-grade BM25 + dense cosine + Reciprocal Rank Fusion (RRF, k=60) + injectable reranker + metadata/recency filters + citation grounding (claim-by-claim Jaccard against retrieved hits with coverage score). Embedding fn and reranker injected by caller so any provider (Cohere/Voyage/OpenAI/SentenceTransformers) can be used. Pure JS, deterministic, zero deps — IMPLEMENTED",
      "Sira Document Pipeline Registry (recommended-stack §Document Intelligence): declarative parser + generator catalogue. PARSERS covers Docling, LlamaParse, MinerU, PyMuPDF, pdfplumber, pypdf, marker, mammoth, python-docx, sheetjs, openpyxl, pandas, python-pptx, markitdown, unified, tesseract, paddleocr, easyocr. GENERATORS covers python-docx, docxtpl, docx-js, docxtemplater, exceljs, xlsxwriter, openpyxl, sheetjs, pptxgenjs, python-pptx, playwright-pdf, weasyprint, reportlab, pdfkit, pdf-lib, wkhtmltopdf, sharp, resvg, canvg, markdown-it, marked, pandoc, tectonic, quarto. chooseParsers / chooseGenerators rank by preference + filter by runtime + capabilities. dispatchParse / dispatchGenerate try providers in order until one succeeds — IMPLEMENTED",
      "Sira LLM Observability (recommended-stack §Observability): Langfuse / Phoenix / Helicone / OpenLLMetry-shaped vocabulary (session → trace → span → generation → score → event). 14 SPAN_KINDS, frozen records, deterministic ids, default redaction of api_key / sk- tokens / authorization headers / passwords / SSNs / credit cards. ObservabilityHub fans out to plug-in sinks (in-memory, console, Langfuse) and tolerates sink failure without breaking the run — IMPLEMENTED",
      "Sira Eval Harness (recommended-stack §Evals): Promptfoo / DeepEval / Ragas-shaped suite of 16 metrics across 4 categories (RAG: faithfulness / answer_relevancy / context_precision / context_recall / context_entities_recall; Agent: tool_call_accuracy / agent_goal_accuracy / task_completion; Safety: hallucination / toxicity / bias / prompt_injection_resistance; Quality: coherence / conciseness / format_compliance / language_compliance). LOWER_IS_BETTER metrics (hallucination / toxicity / bias) handled correctly. evaluateMetric / evaluateSuite / runPromptfooSuite — IMPLEMENTED",
