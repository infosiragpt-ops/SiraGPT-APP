...
function getVisualMediaManifests() {
  return {
    ...
    generate_speech: {
      ...
      side_effect_level: "remote-read",
      scopes: ["ai.audio","files.write"],
      data_classes: ["public"],
    },
    generate_music: {
      name: "generate_music",
      purpose: "Generate an original music track / song from a text description via ElevenLabs Music and save it as a downloadable, playable MP3 artifact.",
      inputs: {
        type: "object", required: ["prompt"],
        properties: {
          prompt: { type: "string", description: "Description of the music: theme, mood, instruments, genre, lyrics direction." },
          durationSeconds: { type: "integer", description: "Target length in seconds (default 30)." },
          genre: { type: "string", description: "Optional genre/style hint (lofi, rock, pop, jazz, cinematic, etc.)." },
        },
      },
      outputs: { type: "object", properties: {
        ok: { type: "boolean" },
      ...
      expected_errors: [
        { code: "no_api_configured", description: "ELEVENLABS_API_KEY not set.", repair_hint: "Configure the ElevenLabs key." },
        { code: "empty_prompt", description: "No prompt provided to generate music." },
        { code: "api_error", description: "The music service returned an error." },
      ],
      acceptance_tests: ["returns ok:true with an mp3 artifact when ELEVENLABS_API_KEY is configured"],
      usage_limits: { timeout_ms_default: 60000, timeout_ms_max: 180000, max_calls_per_task: 3, requires_auth: true, requires_network: true },
      examples_positive: [{ when: "user wants a jingle", call: { prompt: "Upbeat lofi intro jingle", durationSeconds: 20, genre: "lofi" } }],
      examples_negative: [{ when: "user wants spoken narration", why: "use generate_speech instead." }],
      recovery_policy: { on_timeout: "Return ok:false.", on_error: "Surface the error message.", max_retries: 1 },
      side_effect_level: "remote-read",
      scopes: ["ai.audio","files.write"],
      data_classes: ["public"],
    ...
    edit_image: {
      name: "edit_image",
      purpose: "Edit / transform an EXISTING image with a natural-language instruction (img2img): change background, add/remove objects, recolor, restyle, retouch. Not for creating new images (use generate_image).",
      inputs: {
        type: "object", required: ["instruction"],
        properties: {
          instruction: { type: "string", description: "The transformation to apply, in natural language." },
      ...
      forbidden_formats: [],
      expected_errors: [
        { code: "no_source_image", description: "No source image could be resolved.", repair_hint: "Attach an image or pass imageUrl/fileId." },
        { code: "no_provider", description: "No image-edit provider configured." },
        { code: "edit_failed", description: "The edit model returned an error." },
      ],
    ...
    create_swimlane_diagram: {
      ...
      purpose: "Generate a BPM swimlane diagram SVG: a grid of N actor/role lanes × M stages with tasks placed in specific cells and handoff arrows between consecutive tasks. Use for cross-functional process documentation, hand-off mapping, service blueprints, workflow audits.",
      inputs: {
        type: "object", required: ["title","lanes","stages","tasks"],
        properties: {
          title: { type: "string" },
          subtitle: { type: "string" },
          lanes: { type: "array", items: { type: "string" }, description: "2-6 actor/role labels." },
          stages: { type: "array", items: { type: "string" }, description: "2-7 stage labels." },
          tasks: { type: "array", items: { type: "object" }, description: "1-30 tasks: { label, lane, stage, color? }." },
          showHandoffs: { type: "boolean", description: "Draw arrows between consecutive tasks. Default: true." },
          theme: { type: "string", enum: ["professional","modern","minimal","corporate"] },
        },
      },
      outputs: { type: "object", properties: { ok: { type: "boolean" }, downloadUrl: { type: "string" }, id: { type: "string" }, filename: { type: "string" }, lanes: { type: "integer" }, stages: { type: "integer" }, tasks: { type: "integer" } } },
      allowed_formats: ["svg"],
      forbidden_formats: [],
      expected_errors: [
        { code: "empty_lanes", description: "lanes array is empty.", repair_hint: "Provide at least 2 lanes." },
        { code: "empty_stages", description: "stages array is empty.", repair_hint: "Provide at least 2 stages." },
        { code: "empty_tasks", description: "tasks array is empty.", repair_hint: "Provide at least one task with lane/stage indexes." },
      ],
      acceptance_tests: ["returns ok:true for a 3-lane × 4-stage swimlane with 6 tasks"],
      usage_limits: { timeout_ms_default: 15000, timeout_ms_max: 60000, max_calls_per_task: 5, requires_auth: false, requires_network: false },
      examples_positive: [{ when: "user wants a customer-onboarding swimlane", call: { title: "Onboarding", lanes: ["Customer","Sales","Engineering"], stages: ["Sign up","Activate","Use","Renew"], tasks: [{ label: "Fill form", lane: 0, stage: 0 }, { label: "Provision", lane: 2, stage: 1 }] } }],
      examples_negative: [{ when: "user wants RACI responsibility assignment per task", why: "use create_raci_matrix — swimlane shows time-ordered process across actors, RACI shows per-task responsibility roles." }],
      recovery_policy: { on_timeout: "Return ok:false.", on_error: "Surface the error.", max_retries: 1 },
      side_effect_level: "local-fs",
      scopes: ["files.write"],
      data_classes: ["public","internal"],
    ...
    create_mindmap_radial: {
      ...
      purpose: "Generate a radial mindmap SVG: a central topic with 2-8 main branches radiating outward, each with 0-5 sub-branches fanned around it. Use for brainstorming, knowledge organisation, project planning, content outlines. Distinct from create_decision_tree (top-down) and create_concept_map (free graph).",
      inputs: {
        type: "object", required: ["title","centralTopic","branches"],
        properties: {
          title: { type: "string" },
          subtitle: { type: "string" },
          centralTopic: { type: "string", description: "Text in the central root node." },
          branches: { type: "array", items: { type: "object" }, description: "2-8 branches: { label, children?, color? }." },
          theme: { type: "string", enum: ["professional","modern","minimal","corporate"] },
        },
      },
      outputs: { type: "object", properties: { ok: { type: "boolean" }, downloadUrl: { type: "string" }, id: { type: "string" }, filename: { type: "string" }, branches: { type: "integer" }, subtopics: { type: "integer" } } },
      allowed_formats: ["svg"],
      forbidden_formats: [],
      expected_errors: [
        { code: "empty_branches", description: "branches array is empty.", repair_hint: "Provide at least 2 branches." },
        { code: "too_few_branches", description: "mindmap requires at least 2 branches.", repair_hint: "Add at least one more branch." },
      ],
      acceptance_tests: ["returns ok:true for a 4-branch mindmap with children on each"],
      usage_limits: { timeout_ms_default: 15000, timeout_ms_max: 60000, max_calls_per_task: 5, requires_auth: false, requires_network: false },
      examples_positive: [{ when: "user wants a mindmap of a feature scope", call: { title: "Sprint scope", centralTopic: "Sprint 14", branches: [{ label: "Auth", children: ["Login", "OAuth"] }, { label: "Billing", children: ["Stripe", "Refund"] }] } }],
      examples_negative: [{ when: "user wants a free-form node graph with non-hierarchical relations", why: "use create_concept_map — mindmaps assume a single root + tree structure." }],
      recovery_policy: { on_timeout: "Return ok:false.", on_error: "Surface the error.", max_retries: 1 },
      side_effect_level: "local-fs",
      scopes: ["files.write"],
      data_classes: ["public","internal"],
    ...
    create_concept_map: {
      ...
      purpose: "Generate a concept map SVG: an undirected node-link graph laid out radially with 2-12 concept nodes and labelled edges representing semantic relations. Optional per-node category for colour grouping. Use for knowledge maps, taxonomies, semantic networks, ontology sketches.",
      inputs: {
        type: "object", required: ["title","nodes"],
        properties: {
          title: { type: "string" },
          subtitle: { type: "string" },
          nodes: { type: "array", items: { type: "object" }, description: "2-12 nodes: { id, label, category? }." },
          edges: { type: "array", items: { type: "object" }, description: "0-30 edges: { from, to, label? }. Invalid ids are dropped." },
          theme: { type: "string", enum: ["professional","modern","minimal","corporate"] },
        },
      },
      outputs: { type: "object", properties: { ok: { type: "boolean" }, downloadUrl: { type: "string" }, id: { type: "string" }, filename: { type: "string" }, nodes: { type: "integer" }, edges: { type: "integer" }, categories: { type: "integer" } } },
      allowed_formats: ["svg"],
      forbidden_formats: [],
      expected_errors: [
        { code: "too_few_nodes", description: "concept map requires at least 2 nodes.", repair_hint: "Add at least one more node." },
        { code: "too_many_nodes", description: "concept map supports at most 12 nodes.", repair_hint: "Reduce nodes to 12 — past that the radial layout collapses." },
      ],
      acceptance_tests: ["returns ok:true for a 6-node concept map with at least 4 edges"],
      usage_limits: { timeout_ms_default: 15000, timeout_ms_max: 60000, max_calls_per_task: 5, requires_auth: false, requires_network: false },
      examples_positive: [{ when: "user wants a concept map of AI agent components", call: { title: "AI agent", nodes: [{ id: "p", label: "Planner" }, { id: "e", label: "Executor" }, { id: "m", label: "Memory" }], edges: [{ from: "p", to: "e", label: "delegates" }, { from: "e", to: "m", label: "writes" }] } }],
      examples_negative: [{ when: "user wants a parent-child organisational chart", why: "use create_organigram — concept maps are general graphs, not strict hierarchies." }],
      recovery_policy: { on_timeout: "Return ok:false.", on_error: "Surface the error.", max_retries: 1 },
      side_effect_level: "local-fs",
      scopes: ["files.write"],
      data_classes: ["public","internal"],
    ...
    create_moscow_chart: {
      ...
      inputs: {
        ...
        properties: {
          ...
          wontHave:   { type: "array", items: { type: "string" } },
          theme: { type: "string", enum: ["professional","modern","minimal","corporate"] },
        },
      },
      outputs: { type: "object", properties: { ok: { type: "boolean" }, downloadUrl: { type: "string" }, id: { type: "string" }, filename: { type: "string" }, counts: { type: "object" }, total: { type: "integer" } } },
      ...
      forbidden_formats: [],
      expected_errors: [
        { code: "empty_moscow", description: "all 4 priority buckets are empty.", repair_hint: "Provide at least one item in any of mustHave/shouldHave/couldHave/wontHave." },
        { code: "invalid_input_types", description: "one bucket is not an array.", repair_hint: "Pass arrays of strings for each bucket." },
      ],
      acceptance_tests: ["returns ok:true for a MoSCoW chart with items across any column"],
      usage_limits: { timeout_ms_default: 15000, timeout_ms_max: 60000, max_calls_per_task: 5, requires_auth: false, requires_network: false },
      examples_positive: [{ when: "user wants to triage an MVP backlog", call: { title: "MVP", mustHave: ["Auth"], shouldHave: ["Email verification"], couldHave: ["Dark mode"], wontHave: ["Mobile apps"] } }],
      examples_negative: [{ when: "user wants an urgent × important sort", why: "use create_eisenhower_matrix — MoSCoW is priority-bucket categorical, Eisenhower is 2-axis quadrant." }],
      recovery_policy: { on_timeout: "Return ok:false.", on_error: "Surface the error.", max_retries: 1 },
      side_effect_level: "local-fs",
      scopes: ["files.write"],
      data_classes: ["public","internal"],
