...
const BUILTIN_SKILLS = [
  ...
  },
  {
    ...
    body: [
      '# Skill: backend-real',
      '',
      'El starter full-stack YA trae Express + SQLite integrado (server/db.js: bun:sqlite en el preview, node:sqlite en un export — cero dependencias nativas) + Vite con proxy /api. Los datos persisten de verdad: recarga la página y siguen ahí.',
      '',
      '## Arquitectura',
      '- `server/index.js` — servidor Express; `server/db.js` — driver SQLite (usa `db.exec(sql)` y `db.prepare(sql).all/get/run`). Modifica index.js para añadir entidades/rutas.',
      '- `/api/*` — proxy de Vite al Express. En el FRONTEND llama SIEMPRE vía la constante `API` de src/App.tsx (`import.meta.env.BASE_URL` + "/api") — un fetch("/api/...") pelado se sale de la base del preview y da 404.',
      ...
      '4. En el frontend, consume la API con `fetch(\`\${API}/<entidad>\`)` (la constante API ya existe en App.tsx) — nunca uses localStorage para datos que el backend maneja ni un "/api/..." pelado.',
      '',
      '## Calidad',
