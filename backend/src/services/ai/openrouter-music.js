 ...
 * openrouter-music
 *
 * Music generation via OpenRouter audio-capable models. OpenRouter exposes
 * music generation ONLY through Google Lyria (`google/lyria-3-pro-preview`,
 * plus the clip variant) — there are no Suno/Udio models on OpenRouter.
 * Fully-qualified custom slugs are accepted for forward compatibility.
 *
 * Contract: audio output REQUIRES streaming. POST /chat/completions with
 * modalities:["text","audio"], audio:{format:"mp3"} and stream:true;
 * the MP3 arrives as base64 in `choices[].delta.audio.data` across SSE
 * chunks — concatenate and base64-decode to the final file.
 *
 * Mirrors `elevenlabs-music.js` (same audio dir + file naming + return
 * shape) so `/api/ai/generate-music` can pick it transparently.
 * Model selection lives in `music-model-registry.js`.
 */
...
const OPENROUTER_TIMEOUT_MS = Number(process.env.OPENROUTER_MUSIC_TIMEOUT_MS)
  || Number(process.env.LYRIA_TIMEOUT_MS)
  || 240000;
...
function isOpenRouterMusicConfigured() {
  return Boolean(process.env.OPENROUTER_API_KEY);
}

function generatedMusicFilename(prefix = 'openrouter-music', extension = 'mp3') {
...
async function generateOpenRouterMusicFile({ prompt, durationSeconds, model, fetchImpl, signal } = {}) {
  ...
  if (!isOpenRouterMusicConfigured()) {
  ...
  const slug = String(model || '').trim() || LYRIA_MODEL;
  const isLyria = slug.toLowerCase().includes('lyria');
  ...
  const resp = await doFetch(`${OPENROUTER_BASE}/chat/completions`, {
    method: 'POST',
    signal: signalWithTimeout(signal, OPENROUTER_TIMEOUT_MS),
    headers: {
  ...
  if (!resp.ok) {
    ...
    const err = new Error(`OpenRouter music error ${resp.status}: ${String(detail).slice(0, 300)}`);
    err.code = classifyOpenRouterError(resp.status, detail);
    err.status = resp.status;
    throw err;
  }
  if (!resp.body) {
    const err = new Error('OpenRouter music stream body missing');
    err.code = 'API_ERROR';
    throw err;
  }
  ...
  if (streamError && audioParts.length === 0) {
    ...
    const err = new Error(`OpenRouter music stream error: ${detail.slice(0, 300)}`);
  ...
  if (!audioBuffer || audioBuffer.length === 0) {
    const err = new Error('OpenRouter music returned no audio');
    err.code = 'EMPTY_AUDIO';
    throw err;
  }

  ensureDir(audioDir);
  const filename = generatedMusicFilename(isLyria ? 'lyria' : 'openrouter-music');
  ...
  return {
    ...
    model: slug,
    modelLabel: isLyria ? 'Lyria 3 Pro' : slug,
    modelKey: isLyria ? 'lyria' : 'custom',
  };
}

module.exports = {
  generateOpenRouterMusicFile,
  isOpenRouterMusicConfigured,
  clampSeconds,
