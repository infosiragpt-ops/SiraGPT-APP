...
const { resolveModelForUser } = require('../services/model-fallback');
const { FREE_DEFAULT_MODEL } = require('../services/plan-credits');
...
router.get('/models', optionalAuth, responseCache({ ttlMs: 5 * 60_000, namespace: 'ai-models' }), async (req, res) => {
  try {
    ...
    // Always expose the free-tier Gema4 model (virtual when not seeded)
    if (wantText) {
      const hasGema = models.some((m) => m.name === FREE_DEFAULT_MODEL.name);
      if (!hasGema) {
        models = [
          {
            id: '__virtual_gema4_31b__',
            ...FREE_DEFAULT_MODEL,
          },
          ...models,
        ];
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Transparent model fallback — keeps UI selection unchanged while
      // routing exhausted premium users to Gema4 31B.
      if (isAuth) {
        ...
        if (resolved.fallbackApplied) {
          ...
          res.setHeader('X-SiraGPT-Model-Fallback', resolved.reason || 'true');
        }
      }
