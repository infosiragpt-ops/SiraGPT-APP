...
const { searchScholarlyArticles } = require('../services/thesis-citation-engine');
...
async function searchMultipleSources(topic, sessionId) {
  ...
  // OpenAlex — verified scholarly articles (2020+) with DOI / APA 7 refs
  try {
    const scholarly = await searchScholarlyArticles(topic, { perPage: 15 });
    for (const article of scholarly) {
      searchResults.push({
        source: 'OpenAlex',
        title: article.title,
        ...
        doi: article.doi,
        apa7: article.apa7,
        sciteUrl: article.sciteUrl,
        url: article.doi ? `https://doi.org/${article.doi}` : article.openAlexId,
        abstract: '',
        detailedContent: article.apa7,
        type: 'academic',
      });
    }
    console.log(`  ✅ OpenAlex returned ${scholarly.length} verified articles`);
  } catch (openAlexErr) {
    console.warn('  ⚠️ OpenAlex citation fetch failed:', openAlexErr?.message || openAlexErr);
  }
