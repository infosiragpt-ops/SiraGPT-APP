...
const { computePlanCreditTotals, getPremiumCreditsForPlan } = require('../services/plan-credits');
  ...
  try {
    ...
    try {
      ...
      if (session.payment_status === 'paid' && payment.status === 'PENDING') {
        ...
        const creditTotals = computePlanCreditTotals(currentUser, payment.plan);

        const updatedUser = await prisma.user.update({
          where: { id: req.user.id },
          data: {
            plan: payment.plan,
            monthlyLimit: creditTotals.monthlyLimit,
            gemmaTokenPool: creditTotals.gemmaTokenPool,
            stripeSubscriptionId: session.subscription,
...
async function handleCheckoutSessionCompleted(session) {
  try {
    ...
    const creditTotals = computePlanCreditTotals(currentUser, plan);
