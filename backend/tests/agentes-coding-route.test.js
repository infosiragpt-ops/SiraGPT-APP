...
test('non-health routes are 404 while the flag is off', async () => {
  delete process.env.AGENTES_CODING_V2;
  const app = buildApp();
  for (const path of ['/api/agentes-coding/capabilities', '/api/agentes-coding/sessions', '/api/agentes-coding/sandbox']) {
    const res = await request(app).get(path);
    assert.equal(res.status, 404, path);
  }
});

test('capabilities open with the flag on and name the roadmap', async () => {
  process.env.AGENTES_CODING_V2 = 'true';
  const res = await request(buildApp()).get('/api/agentes-coding/capabilities');
  assert.equal(res.status, 200);
  assert.equal(res.body.enabled, true);
  for (const cap of ['sandbox', 'harness', 'editor', 'terminal', 'diff-review', 'preview', 'git', 'deploy']) {
    assert.ok(res.body.capabilities.includes(cap), `missing capability ${cap}`);
  }
});
