 ...
 *      that branches off `error === 'Free daily queries
...
function makePrismaStub(opts = {}) {
  ...
  const freeQuotaDay = opts.freeQuotaDay ?? new Date().toISOString().slice(0, 10);
  return {
    user: {
      async findUnique(args) {
        findUniqueCalls.push(args);
        return opts.findUniqueResult ?? {
          id: "u-free",
          plan: "FREE",
          freeQuotaDay,
          monthlyCallLimit: 3n,
